...
namespace axon 
{
  ...
  public:
    ...
    const virtual std::vector<std::function<double(double)>>& GetActivationFunctions() const = 0;
    const virtual std::vector<std::function<double(double)>>& GetActivationFunctionDerivatives() const = 0;
  };
}
